 ...
 */
namespace SID\InstantEFT\Cron;

use SID\InstantEFT\Controller\AbstractSID;
...
class CronQuery extends AbstractSID
{
    ...
    protected function doSoapQuery( $orderquery )
    {
        ...
        $sidQueryXsd    = 'https://www.sidpayment.com/api/schemas/sid_order_query_request.xsd';
        $sidResponseXsd = 'https://www.sidpayment.com/api/schemas/sid_order_query_response.xsd';
        ...
        curl_setopt( $queryResponse, CURLOPT_SSL_VERIFYPEER, false );
            ...
            if ( !$sidError ) {
                ...
                foreach ( $sid_order_query_response->data->orders->transaction as $transaction ) {
                    ...
                    $payment = $this->updatePayment( (string) $sid_order_query_response->data['signature'],
                        ...
                        (string) $sid_order_query_response->data->outcome['errorsolution'], (string) $transaction->status,
                        (string) $transaction->country->code, (string) $transaction->country->name,
                        (string) $transaction->currency->code, (string) $transaction->currency->name,
                        (string) $transaction->currency->symbol, (string) $transaction->bank->name,
                        (float) $transaction->amount, (string) $transaction->reference, (string) $transaction->receiptno,
                        (string) $transaction->tnxid, $dateCreated, $dateReady, $dateCompleted );
    ...
    private function processPayment( $order, $transaction )
    {
        if ( $order->getStatus() === \Magento\Sales\Model\Order::STATE_PENDING_PAYMENT ) {
            ...
            if ( $sid_status == 'COMPLETED' ) {
                $order->addStatusHistoryComment( "Cron Query, Transaction has been approved, SID_TNXID: " . $sid_tnxid )->setIsCustomerNotified( false )->save();
                ...
                $order->addStatusHistoryComment( "Cron Query, Transaction has been cancelled, SID_TNXID: " . $sid_tnxid )->setIsCustomerNotified( false )->save();
            }
        } else {
